...
def update_row1(ports_val,types_val,date,size_val):
    ...
    if date[0]==0:
        ...
        fr="1-"+str(12)+"-"+str(2019+((date[0]-1)//12))
    else:
        fr="1-"+str(date[0]-((date[0]//12)*12))+"-"+str(2019+(date[0]//12))
    
    if date[1]%12==0:
        to="30-"+str(12)+"-"+str(2019+((date[1]-1)//12))
    else:
        to="30-"+str(date[1]-((date[1]//12)*12))+"-"+str(2019+(date[1]//12))
    
    waiting,ops,service=upper_text_p1(fr=fr,to=to,ports_sel=ports_val,type_vessel=types_val,size=size_val)
    
    return "{:.1f}".format(waiting)+ " hours", format(ops,","), "{:.1f}".format(service) + " hours",\
        fr, to ,format(size_val[0],","),format(size_val[1],",")
...
def update_graphs(ports_val,types_val,date,size_val):
    ...
    service_g,waiting_g,ratio_g=upper_text_p1(fr=fr,to=to,ports_sel=ports_val,type_vessel=types_val,size=size_val,text_bar=False)
...
def update_gatun(date):
    ...
    lake_g=lake_draught(fr=fr,to=to)
...
def update_emissions_map(date,trigger_val):
    ...
    if date[0]==0:
        ...
        fr="1-"+str(12)+"-"+str(2018+((date[0]-1)//12))
    else:
        fr="1-"+str(date[0]-((date[0]//12)*12))+"-"+str(2018+(date[0]//12))
    
    if date[1]%12==0:
        to="30-"+str(12)+"-"+str(2018+((date[1]-1)//12))
    else:
        to="30-"+str(date[1]-((date[1]//12)*12))+"-"+str(2018+(date[1]//12))
    
    if date[1]==12:
        to="31-12-2018"
    
    if not trigger_val:
        emission_fig=emissions_map(fr=fr,to=to)
    else:
        emission_fig=emissions_map(fr=fr,to=to,trigger=True)
